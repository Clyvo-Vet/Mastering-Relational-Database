...
--     CONTA_TUTOR, CONSENTIMENTO, AGENDAMENTO
--
...
-- Razão: profissional vinculado a uma clínica. CRMV é único nacionalmente.
-- ----------------------------------------------------------------------------
...
CREATE TABLE TUTOR (
    ...
    -- Consentimentos para finalidades adicionais (marketing, dados anonimizados etc.) ficam em CONSENTIMENTO (Java)
...
COMMENT ON COLUMN TUTOR.DS_VERSAO_AVISO IS 'Versão do aviso de privacidade apresentado (ex: v1.0, v1.1) — permite rastreabilidade quando termo é atualizado';
...
-- DECISÃO: usar especialização por tabela (não herança) — mais simples no JPA
--          e EF Core. EVENTO_CLINICO tem dados comuns; tabelas específicas
--          (VACINA, PRESCRICAO, EXAME) detalham conforme tipo.
-- ----------------------------------------------------------------------------
...
CREATE TABLE PRESCRICAO (
    ...
    CONSTRAINT CK_PRESC_ASSINADA     CHECK (ST_ASSINADA IN ('S','N'))
);
...
-- Razão: registra todas as notificações enviadas ao tutor (push, WhatsApp, email).
--        Permite auditoria e evita reenvio.
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- CONTA_TUTOR
-- Domínio: Java (Nikolas) | FK TUTOR
-- Razão: separa credenciais de acesso (login/senha/token) dos dados pessoais.
--        Tutor existe sem conta. Conta só existe quando tutor decide usar portal.
-- DECISÃO: relacionamento 1:1 com TUTOR (UNIQUE em ID_TUTOR), mas Java cria/
--          atualiza apenas esta tabela. Felipe nunca toca aqui.
-- ----------------------------------------------------------------------------
...
--        data, IP, status. Permite revogação e auditoria.
-- ----------------------------------------------------------------------------
...
-- ============================================================================
-- 8. AUDITORIA — LOG_ERRO (obrigatório pelo briefing FIAP)
-- ============================================================================
...
-- ============================================================================
-- 10. VIEWS ÚTEIS (apoiam queries das outras disciplinas)
-- ============================================================================
